...
def _resolve_dataset(dataset_name: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
    ...
    record = storage.find_dataset_by_name(normalized)
    if record:
...
def _resolve_code(code_repo: Optional[str], code_name: Optional[str]) -> Tuple[Optional[str], Optional[str], dict, str]:
    ...
    if resolved_name:
        record = storage.find_code_by_name(resolved_name)
        if record:
...
def compute_model_artifact(
    ...
    *,
    ...
    artifact_id = artifact_id or storage.generate_artifact_id()
    ...
    if dataset_name_hint:
        dataset_record = storage.find_dataset_by_name(dataset_name_hint)
        if dataset_record:
    ...
    code_readme: str = ""
    if code_name_hint:
        code_record = storage.find_code_by_name(code_name_hint)
        if code_record:
