...
def has_example_files(model_info: dict, code_info: dict) -> bool:
    ...
    if model_info:
        ...
        for sibling in siblings:
            ...
            if any(kw in filename for kw in EXAMPLE_KEYWORDS) and any(filename.endswith(ext) for ext in EXAMPLE_EXTENSIONS):
...
def reproducibility(model_info: dict, code_info: dict, model_readme: str) -> Tuple[float, int]:
    """
    Calculate reproducibility score based on example code availability and validity.
