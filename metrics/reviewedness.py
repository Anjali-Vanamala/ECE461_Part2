    ...
    try:
        ...
        for pr in recent_prs:
            ...
            if reviews_response.status_code == 200:
                ...
                if len(reviews) > 0:
                    reviewed_count += 1
                    logger.debug(f"PR #{pr['number']} has {len(reviews)} reviews")
        
        fraction = reviewed_count / len(recent_prs)
        logger.info(f"Reviewed PRs: {reviewed_count}/{len(recent_prs)} ({fraction:.2%})")
...
def reviewedness(code_info: dict) -> Tuple[float, int]:
    """
