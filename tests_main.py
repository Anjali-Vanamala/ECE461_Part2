...
class Test_DynamoDBStorage:
    """Tests for backend.storage.dynamodb module with mocked boto3"""
    ...
    def test_save_and_get_dataset_artifact(self):
        """Test saving and retrieving a dataset artifact"""
        mock_table = MagicMock()
        mock_dynamodb = MagicMock()
        mock_dynamodb.Table.return_value = mock_table

        with patch('backend.storage.dynamodb.boto3') as mock_boto3, \
             patch.dict('os.environ', {'DDB_TABLE_NAME': 'test_table', 'AWS_REGION': 'us-east-2'}):
            ...
            with patch.object(dynamodb_module, 'table', mock_table):
                ...
                # Mock get_item to return empty (new artifact)
                ...
                assert item['artifact_id'] == 'test-id-123'
                assert item['artifact_type'] == 'dataset'

                # Mock get_item to return the saved item
                mock_table.get_item.return_value = {
                    "Item": {
                ...
                # Get artifact
                retrieved = dynamodb_module.get_artifact(ArtifactType.DATASET, "test-id-123")
                assert retrieved is not None
                assert retrieved.metadata.id == "test-id-123"
                assert retrieved.metadata.name == "test-dataset"
                assert retrieved.data.url == "https://example.com/dataset"
    ...
    def test_delete_artifact(self):
        ...
        with patch('backend.storage.dynamodb.boto3') as mock_boto3, \
            ...
            with patch.object(dynamodb_module, 'table', mock_table):
                ...
                mock_table.get_item.return_value = {
                    "Item": {
                        "artifact_id": "delete-id-789",
                        "artifact_type": "dataset",
                        "artifact": {
                            "metadata": {"name": "test", "id": "delete-id-789", "type": "dataset"},
                            "data": {"url": "https://example.com"}
                        }
                    }
                }
                # Mock scan for relationship cleanup (empty)
                mock_table.scan.return_value = {"Items": []}

                # Delete artifact
                result = dynamodb_module.delete_artifact(ArtifactType.DATASET, "delete-id-789")

                assert result is True
                assert mock_table.delete_item.called
                delete_call = mock_table.delete_item.call_args
                assert delete_call[1]['Key']['artifact_id'] == 'delete-id-789'

    def test_artifact_exists(self):
        """Test checking if artifact exists by URL"""
        mock_table = MagicMock()
        mock_dynamodb = MagicMock()
        mock_dynamodb.Table.return_value = mock_table
        ...
        with patch('backend.storage.dynamodb.boto3') as mock_boto3, \
            ...
            with patch.object(dynamodb_module, 'table', mock_table):
                ...
                # Mock scan to return item with matching URL
                ...
                exists = dynamodb_module.artifact_exists(ArtifactType.DATASET, "https://example.com/test")
                assert exists is True

                # Test non-existent
                mock_table.scan.return_value = {"Items": []}
                exists = dynamodb_module.artifact_exists(ArtifactType.DATASET, "https://example.com/notfound")
                assert exists is False

    def test_list_metadata(self):
        """Test listing metadata for an artifact type"""
        mock_table = MagicMock()
        mock_dynamodb = MagicMock()
        mock_dynamodb.Table.return_value = mock_table
        ...
        with patch('backend.storage.dynamodb.boto3') as mock_boto3, \
            ...
            with patch.object(dynamodb_module, 'table', mock_table):
                ...
                # Mock scan to return multiple items
                mock_table.scan.return_value = {
                    "Items": [
                        {
                            "artifact_id": "id1",
                            "artifact_type": "dataset",
                            "artifact": {
                                "metadata": {"name": "dataset1", "id": "id1", "type": "dataset"},
                        ...
                        },
                        {
                            ...
                            "artifact_type": "dataset",
                            "artifact": {
                                "metadata": {"name": "dataset2", "id": "id2", "type": "dataset"},
                ...
                metadata_list = dynamodb_module.list_metadata(ArtifactType.DATASET)
                assert len(metadata_list) == 2
                assert metadata_list[0].name == "dataset1"
                assert metadata_list[1].name == "dataset2"

    def test_query_artifacts(self):
        """Test querying artifacts by name"""
        mock_table = MagicMock()
        mock_dynamodb = MagicMock()
        mock_dynamodb.Table.return_value = mock_table
        ...
        with patch('backend.storage.dynamodb.boto3') as mock_boto3, \
            ...
            with patch.object(dynamodb_module, 'table', mock_table):
                    ...
                    "Items": [
                        {
                            ...
                            "artifact_type": "dataset",
                            "artifact": {
                                "metadata": {"name": "test-dataset", "id": "id1", "type": "dataset"},
                                "data": {"url": "https://example.com/1"}
                            }
                        },
                        {
                            ...
                            "artifact_type": "dataset",
                            "artifact": {
                                "metadata": {"name": "other-dataset", "id": "id2", "type": "dataset"},
                                "data": {"url": "https://example.com/2"}
                            }
                        }
                    ]
                ...
                queries = [ArtifactQuery(name="test-dataset")]
                results = dynamodb_module.query_artifacts(queries)
                assert len(results) == 1
                assert results[0].name == "test-dataset"

    def test_save_and_get_model_artifact_with_rating(self):
        """Test saving and retrieving a model artifact with rating"""
        mock_table = MagicMock()
        mock_dynamodb = MagicMock()
        mock_dynamodb.Table.return_value = mock_table
        ...
        with patch('backend.storage.dynamodb.boto3') as mock_boto3, \
            ...
            with patch.object(dynamodb_module, 'table', mock_table):
                ...
                rating = ModelRating(
                    ...
                    size_score=SizeScore(raspberry_pi=0.5, jetson_nano=0.6, desktop_pc=0.7, aws_server=0.8),
                    size_score_latency=0.0
                )

                # Mock get_item to return empty (new artifact)
                mock_table.get_item.return_value = {}
                # Mock scan for _link_dataset_code (empty results)
                mock_table.scan.return_value = {"Items": []}
                ...
                # Mock get_item to return the saved item
                rating_dict = rating.dict() if hasattr(rating, 'dict') else rating.model_dump()
                mock_table.get_item.return_value = {
                    "Item": {
                        "artifact_id": "model-id-456",
                        "artifact_type": "model",
                        "artifact": {
                            "metadata": {
                                "name": "test-model",
                ...
                # Get artifact
                retrieved = dynamodb_module.get_artifact(ArtifactType.MODEL, "model-id-456")
                assert retrieved is not None
                assert retrieved.metadata.id == "model-id-456"

                # Get rating
                retrieved_rating = dynamodb_module.get_model_rating("model-id-456")
                assert retrieved_rating is not None
                assert retrieved_rating.net_score == 0.85

    def test_save_and_get_code_artifact(self):
        ...
        with patch('backend.storage.dynamodb.boto3') as mock_boto3, \
            ...
            with patch.object(dynamodb_module, 'table', mock_table):
                ...
                assert put_call is not None
                item = put_call[1]['Item']
                assert item['artifact_id'] == 'code-id-789'
                assert item['artifact_type'] == 'code'

    def test_save_and_get_model_rating(self):
        """Test saving and retrieving model ratings"""
        mock_table = MagicMock()
        mock_dynamodb = MagicMock()
        mock_dynamodb.Table.return_value = mock_table
        ...
        with patch('backend.storage.dynamodb.boto3') as mock_boto3, \
            ...
            with patch.object(dynamodb_module, 'table', mock_table):
                ...
                rating = ModelRating(
                    ...
                    tree_score=0.8,
                    tree_score_latency=0.0,
                    size_score=SizeScore(raspberry_pi=0.6, jetson_nano=0.7, desktop_pc=0.8, aws_server=0.9),
                    size_score_latency=0.0
                )

                # Mock get_item to return existing model
                rating_dict = rating.dict() if hasattr(rating, 'dict') else rating.model_dump()
                mock_table.get_item.return_value = {
                    "Item": {
                        "artifact_id": "rating-model-id",
                        "artifact_type": "model",
                        "artifact": {
                            "metadata": {"name": "test-model", "id": "rating-model-id", "type": "model"},
                ...
                # Save rating
                dynamodb_module.save_model_rating("rating-model-id", rating)

                # Verify put_item was called to update
                assert mock_table.put_item.called

                # Mock get_item to return updated rating
                mock_table.get_item.return_value = {
                    "Item": {
                        "artifact_id": "rating-model-id",
                        "artifact_type": "model",
                ...
                # Get rating
                retrieved_rating = dynamodb_module.get_model_rating("rating-model-id")
                assert retrieved_rating is not None
                assert retrieved_rating.net_score == 0.9

    def test_find_dataset_by_name(self):
        ...
        with patch('backend.storage.dynamodb.boto3') as mock_boto3, \
            ...
            importlib.reload(dynamodb_module)
            with patch.object(dynamodb_module, 'table', mock_table):
                # Mock scan to return matching dataset
                mock_table.scan.return_value = {
                    "Items": [{
                        "artifact_id": "found-dataset-id",
                        "artifact_type": "dataset",
                        "artifact": {
                            "metadata": {"name": "MyDataset", "id": "found-dataset-id", "type": "dataset"},
                            "data": {"url": "https://example.com/dataset"}
                        }
                    }]
                }

                result = dynamodb_module.find_dataset_by_name("mydataset")
                assert result is not None
                assert result.artifact.metadata.name == "MyDataset"
                assert result.artifact.metadata.id == "found-dataset-id"

                # Test not found
                mock_table.scan.return_value = {"Items": []}
                result = dynamodb_module.find_dataset_by_name("nonexistent")
                assert result is None

    def test_find_code_by_name(self):
        """Test finding code by name"""
        mock_table = MagicMock()
        mock_dynamodb = MagicMock()
        mock_dynamodb.Table.return_value = mock_table
        ...
        with patch('backend.storage.dynamodb.boto3') as mock_boto3, \
            ...
            importlib.reload(dynamodb_module)
            with patch.object(dynamodb_module, 'table', mock_table):
                # Mock scan to return matching code
                mock_table.scan.return_value = {
                    "Items": [{
                        "artifact_id": "found-code-id",
                        "artifact_type": "code",
                        "artifact": {
                            "metadata": {"name": "MyCode", "id": "found-code-id", "type": "code"},
                            "data": {"url": "https://github.com/example/repo"}
                        }
                    }]
                }

                result = dynamodb_module.find_code_by_name("mycode")
                assert result is not None
                assert result.artifact.metadata.name == "MyCode"
                assert result.artifact.metadata.id == "found-code-id"

                # Test not found
                mock_table.scan.return_value = {"Items": []}
                result = dynamodb_module.find_code_by_name("nonexistent")
                assert result is None

    def test_reset(self):
        """Test resetting all artifacts"""
        mock_table = MagicMock()
        mock_dynamodb = MagicMock()
        mock_dynamodb.Table.return_value = mock_table
        ...
        with patch('backend.storage.dynamodb.boto3') as mock_boto3, \
            ...
            importlib.reload(dynamodb_module)
            with patch.object(dynamodb_module, 'table', mock_table):
                # Mock scan to return items to delete
                mock_table.scan.return_value = {
                    "Items": [
                        {"artifact_id": "id1"},
                        {"artifact_id": "id2"}
    ...
    def test_error_handling_client_error(self):
        """Test error handling for ClientError exceptions"""
        mock_table = MagicMock()
        mock_dynamodb = MagicMock()
        mock_dynamodb.Table.return_value = mock_table
        ...
        with patch('backend.storage.dynamodb.boto3') as mock_boto3, \
            ...
            with patch.object(dynamodb_module, 'table', mock_table):
                ...
                result = dynamodb_module.get_artifact(ArtifactType.DATASET, "error-id")
                assert result is None

                # Test delete_artifact with ClientError on get_item
                mock_table.get_item.side_effect = ClientError(
                    {'Error': {'Code': 'ResourceNotFoundException', 'Message': 'Not found'}},
                    'GetItem'
                )
                result = dynamodb_module.delete_artifact(ArtifactType.DATASET, "error-id")
                assert result is False

                # Test artifact_exists with ClientError
                mock_table.scan.side_effect = ClientError(
                    {'Error': {'Code': 'ResourceNotFoundException', 'Message': 'Not found'}},
                    'Scan'
                )
                result = dynamodb_module.artifact_exists(ArtifactType.DATASET, "https://example.com")
                assert result is False

                # Test list_metadata with ClientError
                mock_table.scan.side_effect = ClientError(
                    {'Error': {'Code': 'ResourceNotFoundException', 'Message': 'Not found'}},
                    'Scan'
                )
                result = dynamodb_module.list_metadata(ArtifactType.DATASET)
                assert result == []
    ...
    def test_get_artifact_nonexistent(self):
        ...
        with patch('backend.storage.dynamodb.boto3') as mock_boto3, \
            ...
            with patch.object(dynamodb_module, 'table', mock_table):
                ...
                result = dynamodb_module.get_artifact(ArtifactType.DATASET, "nonexistent-id")
                assert result is None

    def test_get_artifact_wrong_type(self):
        ...
        with patch('backend.storage.dynamodb.boto3') as mock_boto3, \
            ...
            with patch.object(dynamodb_module, 'table', mock_table):
                ...
                # Mock get_item to return item with different type
                mock_table.get_item.return_value = {
                    "Item": {
                        "artifact_id": "wrong-type-id",
                        "artifact_type": "model",  # Different from requested
                        "artifact": {
                            "metadata": {"name": "test", "id": "wrong-type-id", "type": "model"},
                            "data": {"url": "https://example.com"}
                        }
                    }
                }

                # Request dataset but get model
                result = dynamodb_module.get_artifact(ArtifactType.DATASET, "wrong-type-id")
                assert result is None
